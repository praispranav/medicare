...
export default function ZipCodeForm({ setForm, setFormEnd }) {
  ...
  const incZipFormState = (zip) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        if(medicareAandB === YES){
          navigate({ pathname: ROUTES.short.children.congrats, search: generatorQuery.get() });
        } else {
          navigate({ pathname: ROUTES.short.children.noOffer, search: generatorQuery.get() });
        }
        setLoading(false);
      })
      .catch((error) => {
  ...
  const checkPreviousPageData = () => {
    const data = sessionStorage.getItem(sessionStorageKeys.medicareAandB);
    if (data === null)
      navigate({ pathname: ROUTES.short.children.medicareAandB, search: generatorQuery.get() });
  };
              ...
              <form onSubmit={handleSubmit}>
                ...
                >
                  {loading ? (
                    ...
                    <>
                      View My Offer <img src={next} alt="" />
                    </>
                  )}
                </button>
              </form>
